  ...
  return (
    <>
      ...
      <ul>
        ...
        <li>
          Строит таблицу сценариев «есть меньше» в ккал/сут; связь желаемых кг в
          неделю с суточным минусом или плюсом — в отдельном блоке на главной и в
          статье{" "}
      ...
      <p>
        ...
        . <strong>Вода</strong> — не фиксированная норма «для всех», а бытовой
        ориентир. В статьях и калькуляторе мы используем весовой подход
        30–35{NN}мл/кг как стартовую оценку, но официальные справочники чаще
        дают ориентиры по <strong>общей воде за сутки</strong>, включая
        напитки и воду из еды; см.{" "}
        <a
      ...
      <p>
        ...
        веществ может заметно отличаться от формульного. Для уравнения
        Миффлина — Сан Жеора у отдельных людей отклонение от измеренного обмена
        может составлять около 10{NN}% и более. Формулы разработаны для
        здоровых взрослых и{NB}не применимы напрямую при:
      </p>
      <ul>
      ...
      <p>
        Результаты сервиса{NB}не являются медицинской рекомендацией. В
        перечисленных ситуациях и при любых сомнениях обратитесь{NB}к врачу
        или дипломированному диетологу.
      </p>

      <h2>Персональные данные</h2>
      <p>
        Сервис{NB}не собирает и{NB}не хранит введённые в калькуляторы параметры
        (рост, вес, возраст и т.{NB}п.) на своём сервере: расчёт выполняется в
      ...
      <p>
        ...
        {siteContent.contentRevision}. За редакцию и проверку материалов по
        смыслу и формулам отвечает <strong>редакция DayLab</strong>. Если вы
        заметили фактическую ошибку или хотите предложить улучшение — сообщите
        команде проекта удобным для вас способом.
      </p>
