...
const LISTBOX_CLASS =
  `${LISTBOX_UL_STRETCH} absolute left-0 right-0 top-[calc(100%+0.25rem)] z-[80] max-h-[min(20rem,55vh)] overflow-y-auto rounded-lg border border-border/95 bg-elevated shadow-[0_12px_40px_rgba(0,0,0,0.55)]`;

const OPTION_ROW_BASE =
  `${LISTBOX_LI_STRETCH} block ${LISTBOX_LI_OPTION_PAD} transition-[background-color,color]`;

const OPTION_BTN =
  "block w-full cursor-pointer border-0 bg-transparent p-0 text-left text-sm leading-snug text-fg focus-visible:outline-none";

const OPTION_ROW_ACTIVE = " bg-accent/[0.14] text-fg";

const OPTION_ROW_IDLE = " hover:bg-accent/[0.08]";
          ...
          {ACTIVITY_LEVELS.map((lvl) => {
            ...
            return (
              <li
                key={lvl.id}
                role="presentation"
                className={`${OPTION_ROW_BASE}${isHi ? OPTION_ROW_ACTIVE : OPTION_ROW_IDLE}`}
